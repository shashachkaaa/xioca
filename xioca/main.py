...
async def main():
    ...
    if (restart := db.get("xioca.loader", "restart")):
        try:
            ...
            text = f"<emoji id=5463408862499466706>😎</emoji> <code>Xioca</code> <b>полностью перезагружена!</b>\n<emoji id=5386367538735104399>⌛</emoji> <b>Перезагрузка заняла <code>{int(seconds):2d}</code> сек.</b>" if restart["type"] == "restart" else f"<emoji id=5463408862499466706>😎</emoji> <code>Xioca</code> <b>успешно обновлена!</b>\n<emoji id=5386367538735104399>⌛</emoji> <b>Обновление заняло <code>{int(seconds):2d}</code> сек.</b>"
